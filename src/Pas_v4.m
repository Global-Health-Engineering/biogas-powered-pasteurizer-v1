%% Read Me:
% This file can be used togheter with pas_sim_v4 to calculate the results
% from the global simulations in the thesis.
...
holding_T = 120; % [min] Holding retention time, source: Presentation: Three years of field experience piloting the Anaerobic Digestion Pasteurization Latrine by Forbis-Stokes et al.
...
%This and all section bellow need to be run 3 times, for the three different
%heat exchanger coefficients. 

% HX setting: were manually changed from 30, 100 and 300.
...
for i = range_length:-1:1
    in(i) = Simulink.SimulationInput('Pas_sim_v4');

%Defines multiple simulations with the different energy inputs:
    in(i) = in(i).setBlockParameter('Pas_sim_v4/Q_in',...
       'value',num2str(E_in_range(i)));
end
 out = parsim(in,'ShowSimulationManager','on','ShowProgress','on'); %Runs the before defined simulations and returns results.

%% Data analysis
% Section needs to be run three times


pas_temp_matrix3 = zeros(range_length,2); %first run, name this variable 'pas_temp_matrix1', second run: 'pas_temp_matrix2', third run: 'pas_temp_matrix3'
t90_matrix3 = zeros(range_length,2); %first run, name this variable '...1', second run: '...2', third run: '...3'
t90=0;

for a = 1 : 1 : range_length 
    H_exit_t = getElement(out(1,a).xout,'Pas_sim_v4.Type.Direct.Holding.B.T').Values; %reads temp value after holding
    ...
    pas_temp_matrix3(a,1) = E_in_range(a); %first run, change variable '...1', second run: '...2', third run: '...3'
    pas_temp_matrix3(a,2) = pas_temp - 273.15; % store final pasteurisation temperatures in degree celcius. %first run, change variable '...1', second run: '...2', third run: '...3'
    t90_matrix3(a,1) = E_in_range(a); %first run, change variable '...1', second run: '...2', third run: '...3'
    t90_matrix3(a,2) = t90; %first run, change variable '...1', second run: '...2', third run: '...3'
...
figure
plot(pas_temp_matrix3(:,1), pas_temp_matrix3(:,2)) %takes the results of third run
title('1-1')
...
plot(pas_temp_matrix1(:,1), pas_temp_matrix1(:,2)) %takes the results of first run
hold off
...
figure
plot(t90_matrix3(:,1), t90_matrix3(:,2)) %takes the results of third run
title('t90 vs energy in')
xlabel('Energy in [W]', 'Interpreter', 'tex');
ylabel('Time [s]', 'Interpreter', 'tex');
hold on
plot(t90_matrix2(:,1), t90_matrix2(:,2)) %takes the results of second run
plot(t90_matrix1(:,1), t90_matrix1(:,2)) %takes the results of first run